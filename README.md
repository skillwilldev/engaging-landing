# Web Development Practice Portfolio

This repository represents my **Final Exam** project, showcasing a comprehensive range of front-end development skills. It includes everything from semantic HTML structures and complex forms to a fully responsive, animated landing page.

## 🔗 Live Demo
Check out the live preview of the main Landing Page project here:  
👉 [Engaging Landing Page - Live View](https://skillwilldev.github.io/engaging-landing/landing/)

---

## 📂 Project Overview

The repository is organized into several key modules, each designed to test specific web technologies:

### 1. Engaging Landing Page (Main Project)
A modern, high-conversion landing page built based on a professional **Figma** UI Kit.
* **Technologies:** HTML5, CSS3 (BEM methodology, CSS Grid, Flexbox, Clamp for responsive typography).
* **Animations:** Powered by **GSAP (GreenSock Animation Platform)** and **ScrollTrigger** for smooth, interactive scroll-based entrance effects.
* **Responsiveness:** Fully optimized for mobile, tablet, and desktop viewports using a mobile-first approach.

### 2. Multimedia Integration
A project demonstrating the implementation of various media elements and third-party embeds.
* Customized Video Player (utilizing Autoplay, Muted, Loop, and Poster attributes).
* Native Audio elements with fallback support.
* Responsive YouTube Iframe integration.

### 3. Registration Form
A comprehensive HTML form featuring a wide variety of input types and structural elements.
* **Validation:** Implementation of required fields, min/max length, and specific input types (email, password, date).
* **UI Components:** Radio buttons, checkboxes, select menus, file upload, and textareas.
* **Styling:** Clean, accessible design using `fieldset` and `legend` for semantic grouping.

### 4. Interactive Students Table
A data-heavy component focused on complex table structures and mobile accessibility.
* Uses `rowspan` and `colspan` for multi-level headers.
* **Responsive Design:** On mobile devices, the table transforms into a "Card" layout using CSS pseudo-elements (`data-label`) to maintain readability on small screens.

---

## 🛠 Tech Stack
* **HTML5** – Semantic structure and SEO optimization.
* **CSS3** – Advanced layouts (Grid/Flex), Custom Properties (Variables), and Media Queries.
* **GSAP** – Professional-grade web animations.
* **Google Fonts** – Typography integration (Poppins, Montserrat, Mulish).

---

## 👤 Author
**Melik Papikyan**
* GitHub: [@skillwilldev](https://github.com/skillwilldev/engaging-landing)
